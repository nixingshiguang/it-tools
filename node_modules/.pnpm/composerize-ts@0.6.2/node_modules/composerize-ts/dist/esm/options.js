import { OptionType, SupportedOption } from './types';
import { notImplementedInCompose, notYetImplemented, processBoolean, processLoggingOption, processNetworkOption, processOptionWithArgs, processStorageOption, processUlimitOption, } from './callbacks';
/* eslint-disable */
var OPTIONS = [
...
export var getOption = function (optionName) {
    return AllOptions[optionName] || OPTIONS.find(function (o) { return o.short === optionName; });
};
export var getSupportedOptions = function () {
    var implemented = OPTIONS.filter(function (opt) { return opt.action !== notYetImplemented && opt.action !== notImplementedInCompose; }).map(function (opt) { return new SupportedOption(opt.name, opt.path, opt.short); });
    return implemented;
};
